...
function Footer() {
  return <footer className="site-footer"><div className="footer-brand"><BrandMark /><span>Bradford Chessin</span></div><p>Certified Group Fitness Instructor · HIIT + Zumba · In person + online</p><a href={mailto('Training inquiry from the website')}>Contact Bradford <Arrow /></a></footer>;
}
...
function Hero() {
  return <section className="hero" id="about">
    <div className="hero-copy"><p className="eyebrow hero-eyebrow"><span className="live-dot" /> Now training · Fall 2026</p><h1>Build strength.<br /><em>Find your rhythm.</em></h1><p className="hero-lede">Group fitness that meets you where you are and gets you moving with purpose. Bradford Chessin teaches HIIT and Zumba in person at F45 and online through Google Meet.</p><div className="hero-actions"><Link className="button button-dark" href="/schedule">See live schedule <Arrow /></Link><Link className="text-link" href="/classes">Explore classes <Arrow /></Link></div><div className="hero-meta"><span><b>01</b> Certified group fitness</span><span><b>02</b> Weekly Zumba sessions</span><span><b>03</b> F45 + Google Meet</span></div></div>
...
function TrustStrip() { return <div className="trust-strip" aria-label="Training formats"><span>Group training</span><i /><span>HIIT</span><i /><span>Zumba</span><i /><span>Private coaching</span><i /><span>Google Meet</span></div>; }
...
function SchedulePreview() {
  return <section className="schedule-section page-section" id="schedule-preview"><div className="schedule-header"><SectionIntro eyebrow="02 / Live schedule" title="Same energy, every week."><p>Come for the music, stay for the consistency. The Thursday Zumba rhythm is live and ready to book across the next two months.</p></SectionIntro><Link className="text-link" href="/schedule">View full schedule <Arrow /></Link></div><div className="schedule-list">{sessions.slice(0, 4).map((session) => <ScheduleRow key={session.fullDate} session={session} />)}</div><p className="schedule-note"><span className="live-dot" /> All sessions are coached by {instructor.name}. Times shown in Eastern Time.</p></section>;
}
...
function BookingBanner() { return <section className="booking-banner page-section" id="contact"><div><p className="eyebrow">04 / Ready when you are</p><h2>Make your next hour<br /><em>count.</em></h2></div><div className="booking-banner-right"><p>Reserve a weekly Zumba spot, ask about HIIT, or build a private training plan. Google Meet links are shared after your session is confirmed.</p><Link className="button button-dark" href="/book">Start a booking request <Arrow /></Link><a className="email-line" href={mailto('Training inquiry from the website')}>{instructor.email} <Arrow /></a></div></section>; }

function HomePage() {
  return <><Hero /><TrustStrip /><section className="manifesto page-section"><SectionIntro eyebrow="01 / The approach" title="Fitness that feels like a practice, not a punishment."><p>Every class has a clear purpose, a welcoming pace, and enough room for you to make it your own. We train consistently, celebrate the small wins, and keep the door open for the next person.</p></SectionIntro><div className="manifesto-stats"><div><strong>2</strong><span>ways to train<br />F45 + online</span></div><div><strong>1×</strong><span>weekly Zumba<br />Thursday evenings</span></div><div><strong>2028</strong><span>credential<br />valid through</span></div></div></section><section className="classes-section page-section" id="classes-preview"><div className="section-bar"><p className="eyebrow">02 / Classes</p><Link className="text-link" href="/classes">See every service <Arrow /></Link></div><h2>Choose your<br /><em>kind of hard.</em></h2><ClassGrid limit={2} /></section><SchedulePreview /><CredentialTeaser /><BookingBanner /></>;
}
...
function SchedulePage() { return <><PageHero eyebrow="01 / Live schedule" title={<>Show up. <em>Move well.</em></>} description="A public view of upcoming Zumba sessions with Bradford Chessin. Join in person at F45 Training or online through Google Meet." /><section className="page-section standalone-section"><div className="schedule-callout"><div><span className="live-dot" /> Weekly rhythm</div><strong>Thursdays · 6:30–7:30 PM ET</strong><span>New sessions published across multiple weeks</span></div><div className="schedule-list full-schedule">{sessions.map((session) => <ScheduleRow key={session.fullDate} session={session} />)}</div><p className="schedule-note"><span className="live-dot" /> The schedule is designed to make weekly attendance simple. Reserve a session to receive the exact location or Google Meet link.</p></section><BookingBanner /></>; }
...
function BookingPage() {
  ...
  return <><PageHero eyebrow="01 / Booking" title={<>Find your <em>next session.</em></>} description="Choose a service and send a booking request. Bradford will reply with availability, the exact F45 details, or a Google Meet link for online sessions." /><section className="page-section booking-page-grid"><div className="booking-form-card"><div className="form-card-header"><span>Booking request</span><span>01 / 03</span></div><h2>Let’s get you<br /><em>on the schedule.</em></h2><form action={mailto('New training booking request')} method="post" encType="text/plain" onSubmit={() => setSent(true)}><label htmlFor="service">I’m interested in</label><select id="service" name="service" value={service} onChange={(event) => setService(event.target.value)}>{classOfferings.map((item) => <option key={item.title}>{item.title}</option>)}</select><label htmlFor="format">Preferred format</label><select id="format" name="format" defaultValue="In person at F45"><option>In person at F45</option><option>Online via Google Meet</option><option>Either works</option></select><label htmlFor="message">A little about your goals</label><textarea id="message" name="message" rows={4} placeholder="What would you like to work on?" /><button className="button button-dark button-full" type="submit">Email my request <Arrow /></button></form>{sent && <p className="form-success" role="status">Your email app should open with a draft request. If it doesn’t, email {instructor.email} directly.</p>}</div><div className="booking-side"><div className="booking-side-block"><p className="eyebrow">How it works</p><ol className="booking-steps"><li><strong>Pick a service</strong><span>Choose Zumba, HIIT, or a more personal format.</span></li><li><strong>Send a request</strong><span>Tell Bradford your preferred format and goals.</span></li><li><strong>Get your link</strong><span>Receive confirmation plus the F45 details or Google Meet link.</span></li></ol></div><div className="booking-side-block booking-side-note"><span className="note-mark">✦</span><p>Want a recurring weekly spot? Mention it in your request and Bradford will help you find a rhythm.</p></div><a className="email-line" href={mailto('Training inquiry from the website')}>Prefer email? {instructor.email} <Arrow /></a></div></section></>;
}
