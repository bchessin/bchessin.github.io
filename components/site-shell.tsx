...
function Hero() {
  return <section className="hero" id="about">
    <div className="hero-copy"><p className="eyebrow hero-eyebrow"><span className="live-dot" /> Now training · Weekly schedule</p><p className="hero-identity"><strong>Bradford Chessin</strong><span>— Certified Group Fitness Instructor</span></p><h1>Build strength.<br /><em>Find your rhythm.</em></h1><p className="hero-lede">Group fitness that meets you where you are and gets you moving with purpose. HIIT is coached in person at F45. Zumba is coached online through Google Meet.</p><div className="hero-actions"><Link className="button button-dark" href="/schedule">See live schedule <Arrow /></Link><Link className="text-link" href="/classes">Explore classes <Arrow /></Link></div><div className="hero-meta"><span><b>01</b> Certified group fitness</span><span><b>02</b> Zumba twice weekly</span><span><b>03</b> F45 HIIT M / W / F</span></div></div>
...
function TrustStrip() { return <div className="trust-strip" aria-label="Training formats"><span>F45 HIIT / Mon · Wed · Fri</span><i /><span>Zumba / Tue · Thu</span><i /><span>6:10 PM ET daily</span><i /><span>Adults 18+</span></div>; }
...
function RoleProof() {
  return <section className="role-proof page-section"><div className="role-proof-copy"><p className="eyebrow">04 / Coaching affiliation</p><h2>F45 Coach.<br /><em>In the room.</em></h2><p>F45 coaching is an in-person HIIT role. The badge below supports Bradford’s active coach identity and complements the public weekly schedule. For the strongest verification trail, pair it with the individual studio’s official schedule or coach roster.</p><div className="credential-data role-data"><div><span>Role</span><strong>F45 Coach</strong></div><div><span>Format</span><strong>In person · HIIT</strong></div><div><span>Instructor</span><strong>{instructor.name}</strong></div></div><a className="text-link" href="/schedule">View the live schedule <Arrow /></a></div><div className="role-proof-card"><Image className="role-proof-image" src={f45BadgeImage} alt="F45 Coach badge for Bradford Chessin" width={1409} height={1117} /><span>Badge supplied by Bradford Chessin · F45 marks shown for role identification.</span></div></section>;
}
